...
void mlp_train(MLP *mlp, float *inputs, float *targets, int num_samples) {
    for (int epoch = 0; epoch < NUM_EPOCHS; ++epoch) {
        for (int idx = 0; idx < num_samples; ++idx) {
            ...
            float output = mlp->output_biases[0];
            for (int i = 0; i < HIDDEN_SIZE; ++i) {
            ...
            for (int i = 0; i < HIDDEN_SIZE; ++i) {
                float d_hidden = d_output * mlp->output_weights[i];
                d_hidden = d_hidden * ((hidden[i] > 0) ? 1.0f : 0.0f);  // Derivative of ReLU

                // Update weights and biases
                mlp->output_weights[i] += LEARNING_RATE * d_output * hidden[i];
                for (int j = 0; j < INPUT_SIZE; ++j) {
                    mlp->input_weights[j * HIDDEN_SIZE + i] += LEARNING_RATE * d_hidden * inputs[idx * INPUT_SIZE + j];
                }
                mlp->hidden_biases[i] += LEARNING_RATE * d_hidden;
            }
